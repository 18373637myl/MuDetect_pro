...
public class Miner {
	...
	private int mine(HashSet<Fragment> result, HashSet<Fragment> fragments, Pattern pattern, HashSet<Fragment> frequentFragments) {
		...
		int xfreq = config.minPatternSupport - 1;
		boolean extensible = false;
		Queue<HashSet<Fragment>> frequentGroup = new PriorityQueue<>(comparator); //将group按数量存到有限队列中
		...
		//进行抽象操作，将与频率最高的相近频率的点加入该列表
		ArrayList<Node> prepareForAbstract = new ArrayList<>();
		for (HashSet<Fragment> g : frequentGroup){
			if(!g.equals(group)){
				int freq = computeFrequency(g);
				if(freq >= xfreq/2){
					for(Fragment fragment : g){
						prepareForAbstract.add(fragment.getNodes().get(fragment.getNodes().size()-1));
						break;
					}
				}
			}
		...
		Node freqNode = null;
		for(Fragment fragment : group){
			freqNode = fragment.getNodes().get(fragment.getNodes().size()-1);
			break;
		}
		if(freqNode!=null && freqNode.isCoreAction()){
			for(Node node : prepareForAbstract){
				abstractOperation(freqNode,node);
			}
		}
	...
	private void abstractOperation(Node n1, Node n2){
		...
		String label_e1 = config.labelProvider.getLabel(e1);
		String label_n1 = config.labelProvider.getLabel(n2);
		if(n2.isCoreAction()){
			Edge e2 = n2.getExtendBy();
			String label_e2 = config.labelProvider.getLabel(e2);
			String label_n2 = config.labelProvider.getLabel(n2);
